...
if (requireNamespace("HGNChelper", quietly = TRUE)) {
  ...
  if ("Suggested.Symbol" %in% names(checked)) {
    symbols <- checked$Suggested.Symbol
  }
}
...
ugenes <- unique(symbols)
expr_collapsed <- sapply(ugenes, function(g) {
  rows <- symbols == g
  colMeans(expr[rows, , drop = FALSE], na.rm = TRUE)
})
expr_collapsed <- t(expr_collapsed)
rownames(expr_collapsed) <- ugenes

expr_tpm <- apply(expr_collapsed, 2, function(x) {
  x[x < 0] <- 0
