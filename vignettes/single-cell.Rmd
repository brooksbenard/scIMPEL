...
When original cell type annotations are available, the same representation can be shown for **Celltype (original)** labels.

```{r celltype-original-in-extreme-percentiles}
if (!is.null(celltype_original_col) && celltype_original_col %in% names(meta)) {
...
```{r proportion-by-sample-celltype, fig.height=6}
sample_col <- NULL
